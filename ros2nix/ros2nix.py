...
def generate_fire(args, pkg: NixPackage, derivation_text: str, our_cmd_line: str):
    if args.nixfmt:
    ...
    flake_text = dedent(
        ...
        {{
          ...
          outputs = inputs @ {{ self, fire, ... }}: (
            fire.lib.mkPackage inputs rec {{
              name = "{NixPackage.normalize_name(pkg.name)}";
              version = "{pkg.version}";

              package = (
{indent(derivation_text.rstrip('\n'), "                ")}
              );
            }}
          );
        }}
...
class FireAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        namespace.shell = False
        namespace.default = False
        namespace.overlay = False
        namespace.packages = False
        namespace.fire = True
        namespace.name_param = "name"
        namespace.version_param = "version"
        namespace.src_param = "src"
        namespace.nixfmt = True
...
def ros2nix(args):
    ...
    parser.add_argument(
        ...
        nargs=0,
        action=FireAction,
        help="Generate a FIRE compatible flake.nix file"
    )
    ...
    for source in args.source:
        if (args.fire and len(args.source) != 1):
            err(f'Too many sources passed. Only one package.xml is accepted when using FIRE.')
            return 1;
        ...
        try:
            ...
            if not args.packages and not args.fire:
        ...
        if args.fire:
            try:
                generate_fire(args, pkg, derivation.get_text(args.copyright_holder, args.license), our_cmd_line)
                ok(f"Successfully generated FIRE defintion for '{pkg.name}' as 'flake.nix'.")
